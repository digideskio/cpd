 ...
 * 
 */
function cpdca_add_capabilities() {

	$role = get_role( 'supervisor' );
	$role->add_cap( 'copy_assignments' );
}
add_action( 'admin_init', 'cpdca_add_capabilities' );
...
// Add to journal menus
add_action( 'admin_menu', 'cpdca_add_menu_page' );

// Add to network admin menu
add_action( 'network_admin_menu', 'cpdca_add_menu_page' ); 
 ...
 * 
 */
function cpdca_render_menu_page()
{	
	global $wpdb;
	...
	?>
	<div class="wrap cpdca_options">
		<h2>Copy Assignments</h2>
		<form method="post">
			<?php 
				settings_fields( 'cpdca_group' );
				do_settings_sections( 'cpdca_group' );
