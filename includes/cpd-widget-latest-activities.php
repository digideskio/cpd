...
if ( !class_exists( 'WP_Widget_Recent_Activities' ) ) {
...
class WP_Widget_Recent_Activities extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_recent_activities', 'widget' );
		}
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Recent Activities' );
		...
		/**
		 * Filter the arguments for the Recent Activities widget.
		 *
		 * @since 3.4.0
		 *
		 ...
		 */
		$r = new WP_Query( apply_filters( 'widget_activities_args', array(
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_recent_activities', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'widget_recent_activities', 'widget' );
	}
