...
class CPD_Options_Copy_Assessment {
	...
	public function add_options_page() {
		add_submenu_page( 'cpd_settings_templates', 'Copy Assessments', 'Copy Assessments', 'supervise_users', 'cpd_settings_copy_assessments', array( $this, 'render_options_page' ) );
	}
	...
	public function render_options_page(){
		...
		<div class="wrap cpd_options">
			...
			<form method="post">
					...
					foreach( $sites as $site)
					{
						if( strrpos( $site['path'], '/template-' ) === 0 ) {
							...
							if( count( $pages ) > 0 )
							{
								...
								?>
								<tr>
								<th>Assessments in '<?php echo $current_blog_details->blogname; ?>'</th>
				...
				if( $have_pages )
				{
					submit_button('Copy Assessments');
				}
				?>
			</form>
		</div>
