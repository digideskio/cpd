...
class CPD_Options_Copy_PPD {
	...
	public function add_options_page() {
		add_submenu_page( 'cpd_settings_templates', __('Copy Activites', $this->text_domain), __('Copy Activites', $this->text_domain), 'manage_options', 'cpd_settings_copy_ppds', array( $this, 'render_options_page' ) );
	}
	...
	public function render_options_page(){ 
		...
		$post_valid				= false;
		$sites 					= array();
		$success_message		= '<p><strong>'.__('Success', $this->text_domain).': </strong> '.__('Activities have been copied', $this->text_domain).':</p>';
		...
		if( isset( $_POST['submit'] ) )
		{
			...
			if( !$post_valid )
			{
				...
				$error_message = '<p><strong>'.__('Error', $this->text_domain).':</strong> '.__('All chosen Activities already exist in all chosen Journals and/or Templates.', $this->text_domain).'</p>';
		...
		<div class="wrap cpd_options">
			<h2><?php _e('Copy Activities', $this->text_domain);?></h2>
			<form method="post">
				<?php 
					...
					foreach( $sites as $site)
					{
						if( strrpos( $site['path'], '/template-' ) === 0 ) {
							...
							if( count( $pages ) > 0 )
							{
								...
								{
									?>
									<p><?php _e('Below is a list of Template Activities. Check all of the Activities that you wish to copy into participant Journals.', $this->text_domain);?></p>
									<?php
								...
								?>
								<tr>
								<th><?php _e('Activities in', $this->text_domain);?> '<?php echo wp_title(); ?>'</th>
					...
					if( $have_pages )
					{
						...
						<div class="journal-wrapper">
							...
							<p><?php _e('Check the Journals and Templates you wish to copy the selected Activities into.', $this->text_domain);?></p>
							<p><strong><?php _e('Please note', $this->text_domain);?>:</strong> <?php _e('If Activities with the same name already exists in a Journal or Template the page will not be copied into that journal.', $this->text_domain);?></p>
							<?php
					...
					{
						?>
							<p><strong><?php _e('No activities have been created as yet.', $this->text_domain);?></strong></p>
							<p><?php _e('Please create an activities to continue.', $this->text_domain);?></p>
						<?php
					}
				?>
