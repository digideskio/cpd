...
if( !class_exists( 'CPD_Dashboard_Widget_User_Competency_Categories' ) ) {
 ...
 */
class CPD_Dashboard_Widget_User_Competency_Categories {
	...
	public function add_dashboard_widget() {
		...
		$categories_by_participant_title  	= __('All competency categories by user', $this->text_domain );

		$current_user 					= wp_get_current_user();
		$roles 							= $current_user->roles;

		if( in_array( 'supervisor', $roles ) ) {
			$categories_by_participant_title  	= __('All your participants competency categories', $this->text_domain );
		}

		if( in_array( 'supervisor', $roles ) || is_super_admin( $current_user->ID ) ) {
			wp_add_dashboard_widget(
				'cpd_dashboard_widget_user_competency_categories',
	...
	public function render_dashboard_widget(){
		...
		$template_name 						= 	'cpd-dashboard-widget-user-competency-categories';
	...
	public function config_dashboard_widget() {
		...
		$count 	= 	intval( get_option( 'competency_categories_by_participants_barchart_widget_count' ) );
		...
		if( !isset( $_POST['competency_categories_by_participants_barchart_widget_count'] ) ) {
			?>
			...
			update_option( 'competency_categories_by_participants_barchart_widget_count', $_POST['count'] );
			update_option( 'competency_categories_by_participants_barchart_widget_order', $_POST['order'] == 'desc' ? 'desc' : 'asc' );
		}
	}
}
}